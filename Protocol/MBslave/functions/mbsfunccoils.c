...
/* BEGIN: Modified by l  we010 , 2017/10/8 PN: 如果有告警，响应PDU中的功能码变成0x41*/
...
/* BEGIN: Added by lwe004, 2017/9/14   PN:计算功能码正常响应长度 */
    /*计算线圈输出数量*/
	usCoilsCnt = ( USHORT ) ( ( USHORT ) pubMBPDU[MB_PDU_FUNC_READ_COILCNT_OFF] << 8 );
    usCoilsCnt |= ( USHORT ) ( pubMBPDU[MB_PDU_FUNC_READ_COILCNT_OFF + 1] );
    /*判断线圈数量不是八的倍数*/
	...
	/*正常响应长度，pusMBPDULen=功能码字节数 + 字节数字节+ 线圈状态*/
    ...
    MB_CDECL_SUFFIX
{
    ...
    if( *pubMBPDU > ( MB_PDU_FUNC_WRITE_MUL_SIZE_MIN + MB_PDU_SIZE_MIN ) )
    {
        ...
        {
            /* BEGIN: Added by l  we010 , 2017/10/8   PN: 如果有告警，响应PDU中的功能码变成0x4F*/
            pubFrameCur = &pubMBPDU[MB_PDU_FUNC_OFF];
            if( FALSE == Get_WarnStatus() )
            {
 ...
 /* BEGIN: Added by lwe004, 2017/9/14   PN:计算功能码正常响应长度 */
   /**pusMBPDULen = 功能码所占字节数+ 初始地址所占字节数 + 输出数量所占字节数*/
