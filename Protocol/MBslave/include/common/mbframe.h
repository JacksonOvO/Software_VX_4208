 ...
 * All rights reserved.
 *
 * $Id: mbframe.h,v 1.7 2017/01/08 20:04:28 embedded-solutions.cwalter Exp $
 */
...
/* BEGIN: Added by lwe004, 2017/9/14   PN:计算功能码正常响应长度各段字节数定义 */

#define MB_FUNCTION_CODE_SIZE       ( 1 )       /* 功能码所占字节数 */
#define MB_BYTE_COUNT_SIZE           ( 1 )       /* "字节数"所占的字节数 */
#define MB_REG_VALUE_SIZE            ( 2 )       /* 寄存器值所占字节数 */
#define MB_REG_NUMBER_SIZE           ( 2 )       /*寄存器数量所占字节数*/
#define MB_START_ADDRESS_SIZE        ( 2 )      /*起始地址所占字节数*/
#define MB_OUTPUT_NUMBER_SIZE        ( 2 )      /*输出数量所占字节数*/
#define MB_FILE_RESPONSE_DATA_SIZE   ( 1 )     /*文件响应长度所占字节数*/
#define MB_REFER_TYPE_SIZE             ( 1 )    /*参考类型所占字节数*/
#define MB_SIGNAL_RECORD_DATA_SIZE   ( 2 )     /*单个记录数据所占字节数*/
#define MB_RESPONSE_DATA_SIZE         ( 1 )    /*响应数据长度所占字节数*/
/* END:   Added by lwe004, 2017/9/14 */

/* BEGIN: Added by lwe004, 2017/9/17   PN:功能码错误响应 数组各段起始下标定义*/

#define MB_ERROR_RESPONSE_FUNCTION_CODE_INDEX    ( 0 )   /*错误响应功能码下标*/
#define MB_ERROR_RESPONSE_EXCEPTION_CODE_INDEX    ( 1 )  /*错误响应异常码下标*/
#define MB_ERROR_RESPONSE_FILL_START_INDEX        ( 2 )  /*错误响应填充字段起始下标*/
#define MB_ERROR_RESPONSE_FUNCTION_AND_EXCEPTION_CODE_SIZE  ( 2 )  /*错误响应中功能码和错误码所占字节数*/
