...
#endif 
                    /* BEGIN: Added by lwe004, 2017/9/25   PN:eMBPSerialTxEnable 函数功能分解,只支持APV2 */
#if MBS_SERIAL_API_VERSION == 2
                     /*判断发送是否发生错误*/
...
/* BEGIN: Modified by lwe004, 2017/9/20 PN:无需等待超时*/

/* 不再等待超时时间，因为从协议栈接收到的就是完整的响应帧 */
...
/******************************************************************
*函数名称:RTUFrameReceive
*功能描述:协议栈接收数据
*输入参数:xHdl 一个xMBSInternalHandle 句柄
*输出参数:无
...
eMBErrorCode RTUFrameReceive( xMBSHandle xHdl)
{
    ...
    /*通过xMBSInternalHandle获取xMBPSerialHandle*/
    pxIntHdl = xHdl;
    pxRTUHdl = pxIntHdl->xFrameHdl;
